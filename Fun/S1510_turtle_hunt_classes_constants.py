...
class Prey(turtle.Turtle):
    ...
    def rotate_prey(self, positions):  # turtle will be turned right <degree> degrees. Use negative values for left turns.
        ...
        hunters_positions = positions[1:]
        hunter_within_range = False
        hunter0 = True
        hunter1 = True
        hunter2 = True
        for hunter_position in hunters_positions:

            if distance(hunters_positions[0], prey_position) < 100:
                print("Hunter 0")
                if hunter0:
                    degree += direction(hunters_positions[0], prey_position) + 180
                    if degree > 360:
                        degree -= 360
                    hunter0 = False
            ...
            if distance(hunters_positions[1], prey_position) < 50:
                print("Hunter 1")
                if hunter1:
                    degree += direction(hunters_positions[0], prey_position) + 180
                    if degree > 360:
            ...
            if distance(hunters_positions[2], prey_position) < 50:
                print("Hunter 2")
                if hunter2:
                    degree += direction(hunters_positions[0], prey_position) + 180
                    if degree > 360:
        ...
        if not hunter_within_range:
            self.orientation += degree
            self.orientation %= 360
            # print(self.orientation)
            # print(f'{direction(hunter_position, prey_position)}')

        return degree

        # turn = 0
        # degree = 8  # When the turtle rotates the same amount each turn,  it will just run in a circle. Make this function smarter!
        # self.orientation += degree
        # self.orientation %= 360
        # return degree
...
class1 = Prey  # (red prey) Replace PlayerName1 by your own class name here.
class2 = Prey  # (green prey) For testing your code, replace PlayerName1 by your own class name here. Later replace this by your sparring partner's class name.
